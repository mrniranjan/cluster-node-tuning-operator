...
var _ = Describe("[rfe_id:27363][performance] CPU Management", Ordered, func() {
	...
	testutils.CustomBeforeAll(func() {
		...
		onlineCPUSet, err = nodes.GetOnlineCPUsSet(ctx, workerRTNode, logger)
		Expect(err).ToNot(HaveOccurred())
		cpuID := onlineCPUSet.UnsortedList()[0]
		smtLevel = nodes.GetSMTLevel(ctx, cpuID, workerRTNode, logger)
	...
	BeforeEach(func() {
		...
		onlineCPUSet, err = nodes.GetOnlineCPUsSet(context.TODO(), workerRTNode, logger)
		Expect(err).ToNot(HaveOccurred())
	})
	...
	Describe("Verification of configuration on the worker node", Label(string(label.Tier0)), func() {
		It("[test_id:28528][crit:high][vendor:cnf-qe@redhat.com][level:acceptance] Verify CPU reservation on the node", Label("logtest"), func() {
			...
			logger.Infof("Test Info", "Allocatable CPU %d is less than capacity %d by %d", allocatableCPU, capacityCPU, differenceCPUExpected) 
		...
		It("[test_id:37862][crit:high][vendor:cnf-qe@redhat.com][level:acceptance] Verify CPU affinity mask, CPU reservation and CPU isolation on worker node", func() {
			...
			out, err := nodes.ExecCommand(context.TODO(), workerRTNode, cmd, logger)
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("Verification of cpu manager functionality", Label(string(label.Tier0)), func() {
		...
		DescribeTable("Verify CPU usage by stress PODs", func(ctx context.Context, guaranteed bool) {
			...
			out, err := nodes.ExecCommand(ctx,
				workerRTNode,
				[]string{"/bin/bash", "-c", "ps -o psr $(pgrep -n stress) | tail -1"}, logger,
			)
	...
	Describe("Verification of cpu_manager_state file", Label(string(label.Tier0)), func() {
		...
		When("kubelet is restart", func() {
			It("[test_id: 73501] defaultCpuset should not change", func() {
				...
				cpuManagerCpusetBeforeRestart, err := nodes.CpuManagerCpuSet(ctx, workerRTNode, logger)
				Expect(err).ToNot(HaveOccurred())
				...
				_, _ = nodes.ExecCommand(ctx, workerRTNode, kubeletRestartCmd, logger)
				...
				cpuManagerCpusetAfterRestart, err := nodes.CpuManagerCpuSet(ctx, workerRTNode, logger)
				Expect(err).ToNot(HaveOccurred())
	...
	Describe("Verification that IRQ load balance can be disabled per POD", Label(string(label.Tier0)), func() {
		...
		BeforeEach(func() {
			...
			cpuID := onlineCPUSet.UnsortedList()[0]
			smtLevel = nodes.GetSMTLevel(context.TODO(), cpuID, workerRTNode, logger)
		})
		...
		It("[test_id:36364] should disable IRQ balance for CPU where POD is running", func() {
			...
			defaultSmpAffinitySet, err := nodes.GetDefaultSmpAffinitySet(context.TODO(), workerRTNode, logger)
			Expect(err).ToNot(HaveOccurred())

			onlineCPUsSet, err := nodes.GetOnlineCPUsSet(context.TODO(), workerRTNode, logger)
			Expect(err).ToNot(HaveOccurred())
			...
			Eventually(func() bool {
				...
				out, err := nodes.ExecCommand(context.TODO(), workerRTNode, getActiveIrq, logger)
				Expect(err).ToNot(HaveOccurred())
	...
	Describe("Hyper-thread aware scheduling for guaranteed pods", Label(string(label.Tier1)), func() {
		...
		DescribeTable("Verify Hyper-Thread aware scheduling for guaranteed pods",
			func(ctx context.Context, htDisabled bool, snoCluster bool, snoWP bool) {
				...
				cpuID := onlineCPUSet.UnsortedList()[0]
				smtLevel := nodes.GetSMTLevel(ctx, cpuID, workerRTNode, logger)
				hasWP := checkForWorkloadPartitioning(ctx, logger)
				...
				for _, cpuCount := range cpuCounts {
					...
					Expect(checkPodHTSiblings(ctx, testpod, logger)).To(BeTrue(), "Pod cpu set does not map to host cpu sibling pairs")
	...
	Context("Crio Annotations", Label(string(label.Tier0)), func() {
		...
		BeforeAll(func() {
			...
			defaultCpuNotInSchedulingDomains, err := getCPUswithLoadBalanceDisabled(ctx, workerRTNode, logger)
		...
		Describe("Load Balancing Annotation", func() {
			...
			It("[test_id:73382]cpuset.cpus.exclusive of kubepods.slice should be updated", func() {
				...
				out, err := nodes.ExecCommand(ctx, targetNode, cmd, logger)
				Expect(err).ToNot(HaveOccurred())
		...
		Describe("CPU Quota annotation", func() {
			It("[test_id:72079] CPU Quota interface files have correct values", func() {
				...
				err := getter.Container(ctx, testpod, testpod.Spec.Containers[0].Name, cpuCfg, logger)
				Expect(err).ToNot(HaveOccurred())
				if cgroupV2 {
	...
	Context("Check container runtimes cpu usage", Label(string(label.OpenShift)), func() {
		...
		It("[test_id: 74461] Verify that runc excludes the cpus used by guaranteed pod", func() {
			...
			Eventually(func() error {
				expectedRuntime, err := runtime.GetContainerRuntimeTypeFor(context.TODO(), testclient.ControlPlaneClient, guaranteedPod, logger)
				if err != nil {
			...
			GuPods := getConfigJsonInfo(guaranteedPod, "test", workerRTNode, logger)
			...
			BEPods := getConfigJsonInfo(bestEffortPod, "test", workerRTNode, logger)
...
func getConfigJsonInfo(pod *corev1.Pod, containerName string, workerRTNode *corev1.Node, logger *mylog.TestLogger) []*ContainerConfig {
	...
	output, err := nodes.ExecCommand(context.TODO(), workerRTNode, cmd, logger)
...
func checkForWorkloadPartitioning(ctx context.Context, logger *mylog.TestLogger) bool {
	...
	out, err := nodes.ExecCommand(ctx, workerRTNode, cmd, logger)
...
func checkPodHTSiblings(ctx context.Context, testpod *corev1.Pod, logger *mylog.TestLogger) bool {
...
func getCPUswithLoadBalanceDisabled(ctx context.Context, targetNode *corev1.Node, logger *mylog.TestLogger) ([]string, error) {
...
func checkSchedulingDomains(workerRTNode *corev1.Node, podCpus cpuset.CPUSet, testFunc func(cpuset.CPUSet) error, timeout, polling time.Duration, errMsg string, logger *mylog.TestLogger) {
	Eventually(func() error {
		cpusNotInSchedulingDomains, err := getCPUswithLoadBalanceDisabled(context.TODO(), workerRTNode, logger)
		Expect(err).ToNot(HaveOccurred())
