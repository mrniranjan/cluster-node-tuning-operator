...
var _ = Describe("[rfe_id:27363][performance] CPU Management", Ordered, func() {
	...
	Describe("Verification of cpu_manager_state file", func() {
		...
		When("kubelet is restart", func() {
			It("[test_id: 73501] defaultCpuset should not change", Label("kubeletrestart"), func() {
				By("fetch Default cpu set from cpu manager state file before restart")
				cpuManagerCpusetBeforeRestart, err := nodes.CpuManagerCpuSet(workerRTNode)
				Expect(err).ToNot(HaveOccurred())
				...
				_, _ = nodes.ExecCommandOnNode(kubeletRestartCmd, workerRTNode)
				...
				cpuManagerCpusetAfterRestart, err := nodes.CpuManagerCpuSet(workerRTNode)
