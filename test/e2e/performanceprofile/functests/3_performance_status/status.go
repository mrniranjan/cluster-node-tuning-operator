	...
	Context("ovsDpdk CPUs prerequisites", Label(string(label.OvsDpdk), string(label.OpenShift), string(label.Tier2)), func() {
		It("should report Degraded when workload partitioning is disabled", func() {
			...
			Expect(len(isolatedList)).To(BeNumerically(">=", 2),
				"need at least 2 isolated CPUs to designate as ovsDpdk")
			...
			By("Setting ovsDpdk CPUs on the profile without workload partitioning")
			...
			Expect(conditionMessage).To(ContainSubstring(
				"ovs-dpdk cpus require either Workload Partitioning " +
